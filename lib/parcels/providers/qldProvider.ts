...
const QLD_CADASTRE_URL = process.env.QLD_CADASTRE_URL ?? "";

const CAPABILITIES: ProviderCapability[] = ["parcel-by-point"];

const STUB_MESSAGE = "QLD parcel endpoints are intermittently available. Set QLD_CADASTRE_URL to a validated endpoint (e.g. https://gisservices.information.qld.gov.au/arcgis/rest/services/PlanningCadastre/Cadastre/MapServer/0).";
...
export class QldProvider implements ParcelProvider {
  ...
  async getStatus(): Promise<ProviderStatus> {
    return {
      ...
      configured: Boolean(QLD_CADASTRE_URL),
      live: false
    };
  }
}
